...
def generate_permission_url():
  ...
  """
  params = {}
  params['client_id']     = gmvault_utils.get_conf_defaults().get("GoogleOauth2", "gmvault_client_id", "{{ credentials.google_client_id }}")
...
class CredentialHelper(object):
    ...
    def _get_oauth2_acc_tok_from_ref_tok(cls, refresh_token):
      ...
      params = {}
      params['client_id'] = gmvault_utils.get_conf_defaults().get("GoogleOauth2", "gmvault_client_id", "{{ credentials.google_client_id }}")
      params['client_secret'] = gmvault_utils.get_conf_defaults().get("GoogleOauth2", "gmvault_client_secret", "{{ credentials.google_secret }}")
